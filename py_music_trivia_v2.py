...
# Function to extract cover art and title from an MP3 file
def extract_info(mp3_file):
    ...
    if audiofile.tag:
        ...
        if not title:
            title = "Unknown Title"
        return (title, audiofile.tag.images[0].image_data if audiofile.tag.images else None)
...
def trivia_game():
    ...
    while running:
        ...
        if current_song_path:
            title, cover_art = extract_info(current_song_path)
            ...
            if show_cover and cover_art:
                ...
                screen.blit(cover_image, (300, 150))  # Adjust position of cover art
